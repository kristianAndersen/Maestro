#!/bin/bash
# Maestro Statusline - terminal UI dashboard for Claude Code
...
# 256-color palette
C_AGENT="\033[1;38;5;213m"    # Bold bright magenta — agent name
...
if [ -n "$RATE_5H" ]; then
  ...
  RATE_VIS="5h:${RATE_5H_INT}%"
  if [ -n "$RATE_7D" ]; then
    RATE_7D_INT=${RATE_7D%.*}
    RATE_STR="${RATE_STR} ${DIM}7d:${RATE_7D_INT}%${RESET}"
    RATE_VIS="${RATE_VIS} 7d:${RATE_7D_INT}%"
  fi
fi
