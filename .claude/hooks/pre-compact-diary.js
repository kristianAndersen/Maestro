#!/usr/bin/env bun
// PreCompact Diary Capture Hook for Maestro
// Purpose: Captures working context before context window compaction
// Trigger: PreCompact — fires before Claude Code compacts the context window
// Design: Feeds the existing diary/reflector system as a new trigger source
//         (not a parallel persistence mechanism — see session-persister.js for that)
...
// No MIN_PROMPTS guard — compaction implies significant work happened

function logCapture(entry) {
  try {
    mkdirSync(join(__dirname, '..', 'memory', 'diary'), { recursive: true });
...
function main() {
  ...
  try {
    if (!existsSync(CONTEXT_PATH)) {
      logCapture({ timestamp: now, trigger: 'PreCompact', sessionId: hookInput.session_id || null, turnCount: 0, captured: false, reason: 'no-context' });
      process.exit(0);
    }
    context = JSON.parse(readFileSync(CONTEXT_PATH, 'utf-8'));
  } catch {
    logCapture({ timestamp: now, trigger: 'PreCompact', sessionId: hookInput.session_id || null, turnCount: 0, captured: false, reason: 'context-parse-error' });
    process.exit(0);
  }
  ...
  // Guard: no session start means we can't determine the compaction window
  if (!sessionStart) {
    logCapture({ timestamp: now, trigger: 'PreCompact', sessionId, turnCount: promptCount, captured: false, reason: 'no-session-start' });
    process.exit(0);
  }

  // Parse work log for files modified this session
  const filesModified = parseWorkLog(sessionStart);

  // Build staging object — same schema as diary-capture.js with PreCompact extensions
  const stagingData = {
    timestamp: now,
  ...
  const stagingFile = `precompact-${sanitizedTimestamp}.json`;
  const stagingPath = join(STAGING_DIR, stagingFile);

  try {
    writeFileSync(stagingPath, JSON.stringify(stagingData, null, 2), 'utf-8');
  } catch {
    logCapture({ timestamp: now, trigger: 'PreCompact', sessionId, turnCount: promptCount, captured: false, reason: 'write-failed' });
    process.exit(0);
  }
  ...
  logCapture({ timestamp: now, trigger: 'PreCompact', sessionId, turnCount: promptCount, captured: true, stagingFile });

  // Silent exit — PreCompact hooks must not produce output that pollutes the compacted context
}
